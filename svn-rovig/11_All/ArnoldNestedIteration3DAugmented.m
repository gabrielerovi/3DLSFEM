function [solF,WorkingSet] = ArnoldNestedIteration3DAugmented(A,Complementarity,h,bnt1,Constraint,mesh_parameters,Pnt)
...
% Ant1(ii,ii)=1;
% end
[Ant1,bnt1]=add_boundary_bc_system_symmetric(Ant1,bnt1,RemoveNT);
...
WorkingSet_Loc=[];
B=speye(length(bnt1));
% [solC,lambda,WorkingSet] = ArnoldActivesetAugmented(Ant1,B,bnt1,ConstraintC,WorkingSet_Loc,gamma{1});
xold=zeros(length(Ant1),1);
xold(RemoveNT)=bnt1(RemoveNT);
[solC,lambda,WorkingSet] = ArnoldActiveset(Ant1,B,bnt1,ConstraintC,WorkingSet_Loc);
% [solC,lambda,WorkingSet] = ActiveSet(Ant1,B,bnt1,ConstraintC,mesh_parameters{1}.F_contact,xold);
