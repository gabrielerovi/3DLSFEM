...
L=length(mesh);
% gamma = optimalgamma(mesh,parameters)
...
% [graph]=graph_neighb_interior_and_gammac(mesh{L});
[maps,graph]=graph_and_maps(mesh);
% [maps,edgegraphinterior,edgegraphgammac]=mapsedge(maps,mesh);
...
% for ii=1:(mesh{L}.N)-length(mesh{L}.N_contact)
% ii
% nn=graph.graphinterior(ii);
% node=mesh{L}.node(nn,:);
% hold on
% scatter3(node(1),node(2),node(3),'filled')
% pause(0.1)
% end
...
%    mesh_parameters{lev}.Edge_Patch_Internal_All=maps.Edge_Patch_Internal_All{lev};
%    mesh_parameters{lev}.edgegraphgammac=edgegraphgammac{lev};
%    mesh_parameters{lev}.edgegraphinterior=edgegraphinterior{lev};
    if(lev<L)
...
[x,WorkingSetContact{1}] = ArnoldNestedIteration3DAugmented(AFinenobcnt,Complementarity,h,bnt1,c,mesh_parameters,Pnt);
...
ffname{4}='activeset';
ffname{5}='isitlinear';
% A=AFinenobcnt+Complementarity;
% [A,b]=add_boundary_bc_system_symmetric(A,bnt,mesh_parameters{L}.RemoveNT);
...
[x,WorkingSetContact{mm+1},residual(mm)] = ArnoldVcycleContact3DAugmented(AFinenobcnt,Complementarity,bnt,x,h,c,WorkingSetContact{mm},parameters.smoothing_steps,mesh_parameters,graph,Pnt,L,L);
%[x,WorkingSetContact{mm+1},residual(mm)] = ArnoldVcycleContact3DEdge(AFinenobcnt,Complementarity,bnt,x,h,c,WorkingSetContact{mm},parameters.smoothing_steps,mesh_parameters,Pnt,L,L);


%[x,WorkingSetContact{mm+1}]=ArnoldSmootherContactAugmented(WorkingSetContact{mm},A,x,b,c,graph{L},mesh_parameters{L}.N_contact,mesh_parameters{L}.Patch_Internal_All,parameters.smoothing_steps); 
% 
% res=b-A*x;
% res(mesh{L}.RemoveNT)=0;
% res(find(1==WorkingSetContact{mm+1}))=0;
% 
% norm_res(mm)=norm(res);

% if( mod(mm,10)==0)
% ContactPressure(mesh,x,parameters);
% end
[mm,log10(residual(mm))]
...
% 
% save( fullfile( pwd,ffname{4},'activeset.mat'), 'activeset')
% save( fullfile( pwd,ffname{5},'isitlinear.mat'), 'isitlinear')
% save( fullfile( pwd,ffname{1},'x.mat'), 'x')
...
save( fullfile( pwd,ffname{3},'residual.mat'), 'residual')


string1=stringtype;
string1=strcat(string1,string0,'sol/Solutionlambda');
string=strcat(string1,stringext);
save( fullfile( pwd,ffname{2},'sol.mat'), 'sol')
