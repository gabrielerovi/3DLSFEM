function [NE,edge,elemE,E_to_T,N_to_T]=create_edge_structures3D(edge_per_elem,node_per_elem,node,elem)
...
for T=1:NT
   ...
   vertex=sort(elem(T,:));
   edges(1,:)=[vertex(1),vertex(2)];
   edges(2,:)=[vertex(2),vertex(3)];
   edges(3,:)=[vertex(1),vertex(3)];
   edges(4,:)=[vertex(2),vertex(4)];
   edges(5,:)=[vertex(1),vertex(4)];
   edges(6,:)=[vertex(3),vertex(4)];
