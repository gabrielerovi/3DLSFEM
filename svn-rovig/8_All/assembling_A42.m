function M=assembling_A42(qrule,node,edge_per_elem,node_per_elem,alpha,beta,coeff_equilibrium,coeff_constitutive,input_name)
            ...
            for qp=1:number_of_qp
                if( strcmp(input_name,'LSelasticity'))
                    value(qp)= weights(qp)*  (-1.0)* ...
                               coeff_constitutive * (C1 * RT0{ee}(qp,2) * P1grad(nn,2) + C2 * RT0{ee}(qp,1)* P1grad(nn,1)) ;
                elseif(strcmp(input_name,'LSelasticityAsymmetric'))
                    value(qp)= weights(qp)*  (-1.0)* ...
                               coeff_constitutive * (C1 * RT0{ee}(qp,2) * P1grad(nn,2) + C2 * RT0{ee}(qp,1)* P1grad(nn,1)) ;
                end
