function F=assembling_b22(qrule,node,node_per_elem,f,coeff_equilibrium,coeff_constitutive)
...
F=zeros(node_per_elem,1);
   ...
   for nn=1:node_per_elem    
            for qp=1:number_of_qp
                value(qp)= weights(qp) * coeff_constitutive * P1_basis(nn,qp) *f(q_point(1),q_point(2));
            end
