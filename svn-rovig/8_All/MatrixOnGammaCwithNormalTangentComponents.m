function [M_Normal_Tangent,M_Normal_TangentT] = MatrixOnGammaCwithNormalTangentComponents(mesh,householder)
...
for bb=mesh.E_contact
        ...
        phidotn=phi_dot_n(mesh,bb);
         ...
         normal_E=mesh.normal_edge_contact{bb};
...
if(householder==0)
    M_Normal_TangentT=M_Normal_Tangent';
end
...
% normal_edge=mesh{L}.normal_edge_contact;
% 
% [dirichlet,n_and_or_t,bool_bc]= boundary_value_bool(1);
% 
% 
...
%         
%          normal_E=mesh{L}.normal_edge_contact{bb};
%          M_Normal_Tangent( [bb, bb+NE] , :)=0;
%          M_Normal_Tangent( [bb, bb+NE] , [bb, bb+NE] )= 1.0/phidotn * [ normal_E(1)  normal_E(2);
%                                                                        normal_E(2) -normal_E(1);];
%         if(householder)
%         [xn,H]=HouseHolderTransformation(zeros(2,2), normal_E);  
%         M_Normal_Tangent( [bb, bb+NE] , :)=0;
%         M_Normal_Tangent( [bb, bb+NE] , [bb, bb+NE] )= H;
%         M_Normal_TangentT( [bb, bb+NE] , :)=0;
%         M_Normal_TangentT( [bb, bb+NE] , [bb, bb+NE] )= H';
%         end
%         
%         for nn=1:2
%             nn1 = 2 * NE + nodeN(nn);
