...
%  C11=find(c<10);
%  C=B(C11,:);
%  gamma1=1.0/6.0  *parameters.C_contact *6 * (1/2)^(2*(L-1))*0.5;
%  C1=gamma1*C'*C;
%  
%  w=[mesh{L}.F_contact,mesh{L}.N_contact+mesh{L}.NF*3];
%  gamma = optimalgamma(mesh,parameters);
%  C=diag(gamma{L})+Complementarity;
%  Cgamma=C(w,w);
%  Calone=Complementarity(w,w);
% eigA=eig(full(Calone));
% eigC=eig(full(Cgamma));
% [min(eigA),min(eigC)]
% [max(eigA),max(eigC)]
% subplot(2,1,1)
% plot(eigA,'ro')
...
% [max(eigA)-min(eigA),max(eigC)-min(eigC)]
