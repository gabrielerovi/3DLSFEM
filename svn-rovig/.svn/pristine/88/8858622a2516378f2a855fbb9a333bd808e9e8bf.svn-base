...
old_L=size(old_mesh);
old_L=old_L(1);
node=old_mesh{old_L}.node;
elem=old_mesh{old_L}.elem;
...
for lev=1:L
    ...
    mesh{lev} = rmfield(mesh{lev},'flag_for_each_edge');
    mesh{lev} = rmfield(mesh{lev},'flag_for_bc_edge');
    ...
    mesh{lev} = rmfield(mesh{lev},'T_edge_bc');
end
