function M=assembling_C11(qrule,node,node_per_elem,alpha,beta,coeff_equilibrium,coeff_constitutive,coeff_symmetry)
...
C1=coeff_constitutive*beta^2 + 2*coeff_symmetry;
C2=coeff_constitutive*(alpha^2+(alpha+beta)^2);
   ...
   for e1=1:node_per_elem    
        for e2=1:node_per_elem
            ...
            for qp=1:number_of_qp
                value(qp)= weights(qp)*(C1 * P1grad(e1,1)*P1grad(e2,1)'+ C2 * P1grad(e1,2)*P1grad(e2,2));
            end
