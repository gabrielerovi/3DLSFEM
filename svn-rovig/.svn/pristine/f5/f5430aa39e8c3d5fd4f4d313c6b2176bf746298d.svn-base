function [RT0_basis,RT0_divergence] = phiRT2D(q_point,node)
...
% node contains the node of the triangle
    ...
    for qp=1:qrule_npoints
        ...
        RT0_divergence(1)= + sign * dim/(2.0*Area) ;
        RT0_divergence(2)= + sign * dim/(2.0*Area) ;
        RT0_divergence(3)= - sign * dim/(2.0*Area) ;
