function M=assembling_C22(qrule,node,node_per_elem,alpha,beta,coeff_equilibrium,coeff_constitutive,coeff_symmetry)
   ...
   for e1=1:node_per_elem    
        for e2=1:node_per_elem
            ...
            for qp=1:number_of_qp
                value(qp)= weights(qp)*(C1 * P1grad(e1,2)*P1grad(e2,2)+ C2 * P1grad(e1,1)*P1grad(e2,1));
            end
