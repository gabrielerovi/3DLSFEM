function M=assembling_A33(qrule,node,node_per_elem,C1,C2,input_name)
...
%             end
            if( strcmp(input_name,'LSelasticity')||strcmp(input_name,'LSelasticityAsymmetric'))
             q=sum(value) + C2 * (0.5 * P1grad(e1,2)*P1grad(e2,2)+P1grad(e1,1)*P1grad(e2,1) );
            elseif(strcmp(input_name,'DispElasticity'))
                % C1= 2 mu
                % C2 =lambda
              q=sum(value) + C1 * (0.5 * P1grad(e1,2)*P1grad(e2,2)+P1grad(e1,1)*P1grad(e2,1) ) ... 
                           + C2 * (P1grad(e1,1)* ( P1grad(e2,1) ) );  
            end
                 q=q * area;
                 M(e1,e2)=M(e1,e2)+q; 
