...
%connected
%face_connectivity=sparse(N,N,N);
face_connectivity=cell(N,1);
for nn=1:N
    face_connectivity{nn}=sparse(N,N);
end
for T=1:NT
    
   vertex=sort(elem(T,:));
   faces(1,:)=[vertex(2),vertex(3),vertex(4)];
   faces(2,:)=[vertex(1),vertex(3),vertex(4)];
   faces(3,:)=[vertex(1),vertex(2),vertex(4)];
   faces(4,:)=[vertex(1),vertex(2),vertex(3)];
