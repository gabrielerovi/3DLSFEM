...
Given displacement $\bu$, stress $\bsigma$, body $\Omega$:
\begin{align*}
\colk
\footnotesize
\begin{cases}
...
\colk
Given the {\colp gap function g}, the {\colgold normal and tangent vectors $\bn$ and $\bt$ }: 
...
\begin{align*}
&\mathcal{J}(\bbx_L;\bff_L)=\dfrac{1}{2} \bbx_L^T \bA_L \bbx_L - \bbx_L^T \bff_L
\end{align*} 
}
\uncover<5->{\item
...
Find $\bbx_L \in K_L$
\begin{align*}
\text{argmin}\mathcal{J}(\bbx_L;\bff_L)&=\dfrac{1}{2} \bbx_L^T \bA_L \bbx_L -\bbx_L^T \bff_L\\
%\text{such } & \text{that}\\
\bB_L \bbx_L &\leq \bg_L
\end{align*}
}
...
Comparison of coarse function  $\bc_j $ with \textbf{fine constraint} $\Rightarrow$ \textbf{suboptimal complexity}
%\begin{itemize}
...
Comparison of coarse function  $\bc_j $ with \textbf{coarse constraint} $\Rightarrow$ \textbf{optimal complexity}
%\begin{itemize}
